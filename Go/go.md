...
Echo使用go标准http.Cookie对象从处理函数中接收的上下文添加/检索cookie。
...
> Cookie是使用new(http.Cookie)创建的。
> 设置cookie的属性将分配给http.Cookie实例公共属性
> 最后在HTTP响应中c.SetCookie(cookies)添加一个Set-Cookie标头
...
>使用c.Cookie("username")HTTP请求通过名称读取Cookie
>使用Getter函数访问Cookie属性。
...
Echo通过从中间件和处理程序返回错误来提倡集中式HTTP错误处理。集中式错误处理程序允许我们从统一位置将错误记录到外部服务，并向客户端发送自定义HTTP响应。

你可以退回标准error或echo.*HTTPError。

例如，当基本身份验证中间件找到无效凭据时，它返回401 - 未授权错误，中止当前HTTP请求。

```go
e.Use(func(next echo.HandlerFunc) echo.HandlerFUnc{
	return func(c echo.Context)error{
		return echo.NewHTTPError(http.StatusUnauthorized,"Plase provide vaild credentials")
	}
})
```

也可以在echo.NewHTTPError()没有消息的情况下使用，在这种情况下，状态文本将用做错误消息。例如“未经授权”

#### 默认HTTP错误处理程序

Echo提供了一个默认的HTTP错误处理程序，它以JSON格式发送错误。
...
对于标准error，响应发送为500-Internal Server Error;但是，如果您在调试模式下运行，则会发送原始错误消息。如果错误*HTTPError，则使用提供的状态代码和消息发送响应。如果启用了日志记录，则还会记录错误消息。

#### 自定义HTTP错误处理程序

可以通过设置自定义HTTP错误处理程序 e.HTTPErrorHandler

对于大多数情况，默认错误HTTP处理程序应该足够; 但是，如果您想捕获不同类型的错误并采取相应的操作，例如将通知电子邮件或日志错误发送到集中式系统，则自定义HTTP错误处理程序可以派上用场。您还可以向客户端发送自定义响应，例如错误页面或仅JSON响应。

#### 错误页面

以下自定义HTTP错误处理程序显示如何显示不同类型错误的错误页面并记录错误。错误页面的名称应该是CODE.html例如500.html。您可以查看此项目[错误页面](https://github.com/AndiDittrich/HttpErrorPages)以获取预构建的错误页面。
...
>也可以将它们写入外部服务（如Elasticsearch或Splunk),而不是将日志写入记录器。

### 请求（Request）

将请求主体绑定到Go类型使用Context#Bind(i interface{})。默认绑定器支持基于Content-Type标头,解码application/json，application/xml和application/x-www-form-urlencoded数据。
...
可以使用注册CustomBinder,Echo#Binder。
...
表格数据
可以使用名称检索表单数据Context#FormValue(name string)
...
可以使用名称检索查询参数Context#QueryParam(name string)
...
与表单数据类似，可以使用自定义数据类型进行绑定Context#QueryParam(name string)

#### 路径参数

可以使用名称检索已注册的路径参数Context#Param(name string)string。
...
```shell
curl http://localhost:1323/user/joe
```

#### 验证数据

Echo没有内置的数据验证功能，但是你可以使用Echo#Validator和利用第三方库注册自定义验证器。

以下示例使用[validator](https://github.com/go-playgroud/validator)框架进行验证
...
Context#String(code int,s string) 可用于发送带有状态码的纯文本响应。
...
Context#HTML(code int,html string) 可用于发送带状态码的简单HTML响应。如果要发送动态生成的HTML，请参阅模版。
...
Context#HTMLBlob(code int,b []byte)可用于发送带有状态码的HTML Blob。使用输出的模版引擎很方便。

#### 渲染模版

#### 发送JSON

Context#JSON(code int,i interface{})可用于将提供的GO类型编码为JSON，并将其作为响应发送给状态代码。
...
Context#JSON()内部使用json.Marshal可能对大型JSON无效，在这种情况下，可以直接流式传输JSON。
...
Context#JSONPretty(code int,i interface{},indent string)可以用于发送一个JSON响应，它基于缩减很漂亮，可以是空格或制表符。
...
>还可以用Context#JSON()通过pretty在请求URL查询字符串中附加输出带缩进的json。

```shell
curl http://localhost:1323/user/1?pretty
```

#### JSON Blob

Context#JSONBlob(code int,b []byte)可用于直接从外部数据源（数据库）发送预编码的JSON blob。
...
Context#JSONNP(code int,callback string,i interface{})可用于将提供的Go类型编码为JSON，并将其作为回调构造的JSONNP有效负载发送，并使用状态码。

#### Send XML

Context#XML(code int,i interface{})可用于将提供的GO类型编码为XML，并将其作为响应发送到状态。
...
Context#XML内部使用xml.Marshal可能对大型XML无效，这种情况，可以直接流式传输。
...
Context#XMLPretty(code int, i interface{}, indent string) 可以用来发送一个基于缩进打印的XML响应，它可以是空格或制表符。
...
您还可以Context#XML()通过pretty在请求URL查询字符串中附加来输出漂亮的打印XML（缩进空格） 。
...
Context#XMLBlob(code int, b []byte) 可用于直接从外部源（例如，数据库）发送预编码的XML blob。
...
Context#File(file string)可用于发送文件内容作为响应。它会自动设置正确的内容类型并优雅地处理缓存。
...
Context#Attachment(file, name string)类似于File()它用于将文件作为带有提供名称的附件发送。
...
Context#Inline(file, name string)类似于File()它用于发送文件作为内联提供的名称。
...
Context#Blob(code int, contentType string, b []byte) 可用于使用提供的内容类型和状态代码发送任意数据响应。
...
Context#Stream(code int, contentType string, r io.Reader)可用于发送具有提供的内容类型io.Reader和状态代码的任意数据流响应。
...
Context#NoContent(code int) 可用于发送带有状态代码的空体。
...
Context#Redirect(code int, url string) 可用于将请求重定向到具有状态代码的提供的URL。
...
在回应之前
Context#Response#Before(func()) 可用于注册在写入响应之前调用的函数。

回应后
Context#Response#After(func())可用于注册在写入响应后立即调用的函数。如果“Content-Length”未知，则不执行after函数。
...
Echo的路由器基于基数树，使得路由查找非常快。它利用同步池 重用内存，实现零动态内存分配，无需GC开销。

可以通过指定HTTP方法，路径和匹配处理程序来注册路由。
...
可以使用Echo.Any(path string,h Handler)为所以HTTP方法注册处理程序。如果你想注册一些方法使用Echo.Match(mechods []string,path string,h Handler)。

echo定义handler函数为func(echo.Context) error。其中echo.Context主要保持HTTP请求和响应接口。

#### Match-any

匹配路径中的零个或多个字符。例如，模式/users/*将匹配：
/users/
/users/1
...
可以将具有公共前缀的路由分组以定义具有可选中间件的新子路由。除了指定的中间件组，还继承了父中间件。要在组中稍后添加中间件，可以使用Group.Usr(m ...Middleware)。组也可以嵌套。

```go
g:=e.Group("/admin")
g.Use(middleware.BasicAuth(func(usernam,password string,c echo.Context)(bool,error){
...
每个注册方法返回一个Route对象，该对象可用于在注册后命名路由。
...
e.GET("/users/:id",func(c echo.Context)error{}).Name="get-user"
```
...
Echo#URL(handler HandlerFunc,params ...interface{})可用于为具有指定路径参数的任何处理程序生成URL。
...
e.URL(h,1)可为/users/1注册生成。

除此之外Echo#URL，还有Echo#Reverse(name string,params ...interface{})一个用于根据路由名称生成URL。

例如Echo#Reverse("foobar",1234)-> /users/1234
...
Echo#Routes()[]*Route可用于按照定义的顺序列出所有已注册的路由。每个路由都包含HTTP方法，路径和关联的处理程序。
...
#### Static Middleware(静态中间件)

##### Echo#Static(prefix,root string)

注册带路径前缀的新路由，以便从提供的根目录提供静态文件
...
将assets目录中的任何文件提供路径/static/*。例如，一个请求/static/js/main.js将获取并提供assets/js/main.js文件

##### Echo#File()

Echo#File(path,file string)使用路径注册新路由以提供静态文件

提供索引页面public/index.html

```go
e.File("/","public/index.html")
```

供应图标images/favicon.ico
...
Context#Render(code int,name string,data interface{})error使用数据呈现模版并发送带有状态的text/html响应。可以通过设置注册模版Echo.Renderer，允许使用任何模版引擎。
...
func(t *Template)Render(w io.Write,name string,data interface{},c echo.COntext) error{
	return t.templates.ExcuteTemplate(w,name,data)
}
```
...
> 接受JSON负载
> 成功201-Created
> 失败500-Internal Server Error

GetUser
Get /users/:email
>成功200-OK
>失败404-Not Found如果用户未找到500-Internal Server Error
